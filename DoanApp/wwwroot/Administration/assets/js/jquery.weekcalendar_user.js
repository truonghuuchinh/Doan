...
(function ($) {
      ...
      return {
        options: {
          ...
          eventHeader: function (calEvent, calendar) {
            ...
            var displayTitleWithTime =
              calEvent.end.getTime() - calEvent.start.getTime() <=
              one_hour / options.timeslotsPerHour;
        ...
        prevWeek: function () {
          ...
          var newDate = new Date(
            this.element.data("startDate").getTime() - MILLIS_IN_WEEK / 6
          );
          this._clearCalendar();
        ...
        nextWeek: function () {
          ...
          var newDate = new Date(
            this.element.data("startDate").getTime() +
              MILLIS_IN_WEEK +
              MILLIS_IN_DAY
          );
          this._clearCalendar();
        ...
        getTimeslotTimes: function (date) {
          ...
          for (var i = 0; i < options.timeslotsPerDay; i++) {
            ...
            times[i] = {
              ...
              startFormatted: this.formatTime(
                new Date(startMillis),
                options.timeFormat
              ),
              end: new Date(endMillis),
              endFormatted: this.formatTime(
                new Date(endMillis),
                options.timeFormat
              ),
            };
        ...
        getCurrentLastDay: function () {
          return this._addDays(
            this.getCurrentFirstDay(),
            this.options.daysToShow - 1
          );
        },
        ...
        _setOption: function (key, value) {
          ...
          if (self.options[key] != value) {
            ...
            var currentEvents = self.element
              .find(".wc-cal-event")
              .map(function () {
            ...
            self._renderEvents(
              { events: currentEvents, options: newOptions },
        ...
        _resizeCalendar: function () {
          ...
          if (options && $.isFunction(options.height)) {
            ...
            var scrollContainerHeight = Math.max(
              calendarHeight - navHeight - headerHeight,
              options.minBodyHeight
            );
            var timeslotHeight = this.element
              .find(".wc-time-slots")
              .outerHeight();
            this.element
              .find(".wc-scrollable-grid")
              .height(scrollContainerHeight);
            if (timeslotHeight <= scrollContainerHeight) {
              this.element.find(".wc-scrollbar-shim").width(0);
            } else {
              this.element
                .find(".wc-scrollbar-shim")
                .width(this._findScrollBarWidth());
            }
        ...
        _setupEventDelegation: function () {
          ...
          this.element
            .click(function (event) {
              ...
              freeBusyManager = self.getFreeBusyManagerForEvent(
                $calEvent.data("calEvent")
              );

              if (
                options.allowEventDelete &&
                $target.hasClass("wc-cal-event-delete")
              ) {
                options.eventDelete(
                  $calEvent.data("calEvent"),
                  $calEvent,
        ...
        _isDraggingOrResizing: function ($target) {
          return (
            $target.hasClass("ui-draggable-dragging") ||
        ...
        _renderCalendarButtons: function ($calendarContainer) {
          ...
          if (options.buttons) {
            ...
            calendarNavHtml +='<div class="ui-widget-header wc-toolbar block__header">';
            calendarNavHtml += '<div class="wc-display block__button"></div>';
                calendarNavHtml += '<div  style="display: none" class="wc-nav block__button-next">';
                  calendarNavHtml +='<button id="last_week" class="wc-prev">' +options.buttonText.lastWeek + "</button>";
                  calendarNavHtml += '<button id="today_press"s class=\"wc-today\">' + options.buttonText.today + '</button>';
                // calendarNavHtml += '<button class="wc-title"></button>';
                  calendarNavHtml += '<button id="next_week" class="wc-next">' + options.buttonText.nextWeek + "</button>";
                calendarNavHtml += "</div>";
              // calendarNavHtml += '<h1 id="title__week" class=\"wc-title\"></h1>';
              calendarNavHtml += '<div class="header-btn">';
                calendarNavHtml += '<div class="jqyc-year-chooser border-top border-bottom row align-items-center jqyc-week-chooser">';
                  calendarNavHtml += '<label onclick="remove__class()" for="last_week" type="button" class="btn btn-sm bg-white jqyc-prev-year jqyc-change-year">';
                    calendarNavHtml += '<img alt="" src="../assets/img/ic_select_left_blue_24px.svg">';
                  calendarNavHtml += '</label>';
                calendarNavHtml += '<div id="week__title" class=\"wc-title jqyc-year col jqyc-week\"></div>';
                  calendarNavHtml += '<label for="next_week" type="button" class="btn btn-sm bg-white jqyc-next-year jqyc-change-year">';
                    calendarNavHtml += '<img alt="" src="../assets/img/ic_select_right_blue_24px.svg">';
                  calendarNavHtml += '</label>';
                calendarNavHtml += '</div>';
                calendarNavHtml += '<div class="choose-day-now">';
                  calendarNavHtml += '<label  for="today_press" class="btn-choose-day-now user_chose_today">Chọn ngày hôm nay</label>';
                calendarNavHtml += '</div>';
              calendarNavHtml += '</div>';
              // calendarNavHtml += '<div class="wc-nav today__button">';
              //   calendarNavHtml +='<button id="today_press" class="wc-today">' + options.buttonText.today + "</button>";
              // calendarNavHtml += "</div>";
            calendarNavHtml += "</div>";
            $(calendarNavHtml).appendTo($calendarContainer);

            $calendarContainer
              .find(".wc-nav .wc-today")
              .button({
                icons: { primary: "today__button" },
              })
              .click(function () {
                self.today();
                return false;
            ...
            $calendarContainer
              ...
              .button({
                text: false,
                icons: { primary: "fa fa-caret-left" },
              })
              .click(function () {
            ...
            $calendarContainer
              ...
              .button({
                text: false,
                icons: { primary: "fa fa-caret-right" },
              })
              .click(function () {
        ...
        _renderCalendarHeader: function ($calendarContainer) {
          ...
          for (var i = 1; i <= options.daysToShow; i++) {
            calendarHeaderHtml +=
              '<td class="wc-day-column-header wc_userday wc-day-' +
              i +
              '"' +
              colspan +
              "></td>";
          ...
          calendarHeaderHtml +=
            '<td class="wc-scrollbar-shim"' + rowspan + "></td></tr>";
            ...
            for (var i = 1; i <= options.daysToShow; i++) {
              for (var j = 0; j < uLength; j++) {
                ...
                calendarHeaderHtml +=
                  '<td class="' +
                  _headerClass +
                  " wc-user-header wc-day-" +
                  i +
                  " wc-user-" +
                  self._getUserIdFromIndex(j) +
                  '">';
        ...
        _renderCalendarBody: function ($calendarContainer) {
          ...
          $calendarContainer
            .find(".wc-full-height-column")
            .height(options.timeslotHeight * options.timeslotsPerDay- 2.5);
          //set the timeslot height
          $calendarContainer
            .find(".wc-time-slot")
            .height(options.timeslotHeight - 2.5); //account for border
        ...
        */
          ...
          if (showAsSeparatedUser) {
            for (var i = 0, uLength = options.users.length; i < uLength; i++) {
              $calendarContainer
                .find(".wc-user-" + self._getUserIdFromIndex(i))
        ...
        _renderCalendarBodyTimeSlots: function ($calendarTableTbody) {
          var options = this.options,
            ...
            showAsSeparatedUser =
              ...
              options.users &&
              options.users.length,
            start = options.businessHours.limitDisplay
              ? options.businessHours.start
              : 8,
            end = options.businessHours.limitDisplay
          ...
          renderRow +=
            '<td class="wc-grid-timeslot-header"' + rowspan + "></td>";
          renderRow +=
            '<td colspan="' +
            options.daysToShow *
              (showAsSeparatedUser ? options.users.length : 1) +
            '">';
          renderRow +=
          ...
          for (i = 8; i < 19; i++) {
            if(i == 12){
              renderRow += '<div class="wc-time-slot hour__time_'+i+'"></div>';
            }else
            {
              for (j = 0; j < options.timeslotsPerHour - 1; j++) {
                renderRow += '<div class="wc-time-slot hour__time_'+i+'"></div>';
              }
              renderRow += '<div class="wc-time-slot wc-hour-end hour__time_'+i+'"></div>';
            }
          }
        ...
        _renderCalendarBodyOddEven: function ($calendarTableTbody) {
          if (this.options.displayOddEven) {
            var options = this.options,
              ...
              oddEvenClasses = {
                odd: "wc-column-odd",
                even: "ui-state-hover wc-column-even",
              };
            ...
            for (var i = 1; i <= options.daysToShow; i++) {
              if (!showAsSeparatedUser) {
                ...
                renderRow +=
                  '<div class="wc-no-height-wrapper wc-oddeven-wrapper">';
                renderRow +=
                  '<div class="wc-full-height-column ' +
                  oddEvenClasses[oddEven] +
                  '"></div>';
                renderRow += "</div>";
                renderRow += "</td>";
              } else {
            ...
            for (var i = 1; i <= options.daysToShow; i++) {
              if (options.displayFreeBusys) {
                if (!showAsSeparatedUser) {
                  renderRow += '<td class="wc-day-column day-' + i + '">';
                  renderRow +=
                    '<div class="wc-no-height-wrapper wc-freebusy-wrapper">';
                  renderRow +=
                    '<div class="wc-full-height-column wc-column-freebusy wc-day-' +
                    i +
                    '"></div>';
                  renderRow += "</div>";
                  renderRow += "</td>";
                  ...
                  for (var j = 0; j < uLength; j++) {
                    ...
                    renderRow +=
                      ...
                      i;
                    renderRow +=
                      " wc-user-" + self._getUserIdFromIndex(j) + '">';
                    renderRow += "</div>";
                    renderRow += "</div>";
                    renderRow += "</td>";
                  }
        ...
        _renderCalendarBodyEvents: function ($calendarTableTbody) {
          var self = this,
            ...
            renderRow,
            showAsSeparatedUser =
              options.showAsSeparateUsers && options.users && options.users.length, 
              start = options.businessHours.limitDisplay ? options.businessHours.start : 8,
              end = options.businessHours.limitDisplay
              ? options.businessHours.end
              : 18;
          renderRow = '<tr class="wc-grid-row-events">';
          renderRow += '<td style="display: block;" class="wc-grid-timeslot-header">';
          for (var i = 8; i < 19; i++) {
            var bhClass =
              options.businessHours.start <= i && options.businessHours.end > i ? "ui-state-active wc-business-hours" : "ui-state-default";
            renderRow += '<div style="border-left: 1px solid #D8D8D8 !important;border: none;" class="wc-hour-header hour__'+i+' ' + bhClass + '">';
            if (options.use24Hour) {
              renderRow +=
                '<div class="wc-time-header-cell hour_underline hour__block__'+i+'">' +
                self._24HourForIndex(i) +
                "</div>";
            } else {
              renderRow +=
                '<div class="wc-time-header-cell">' +
                self._hourForIndex(i) + '<span class="wc-am-pm">' + self._amOrPm(i) + "</span></div>";
            }
            renderRow += "</div>";
          }
          renderRow += "</td>";
          ...
          for (var i = 1; i <= options.daysToShow; i++) {
            if (!showAsSeparatedUser) {
              renderRow += '<td id="day__'+i+'" class="' + _columnBaseClass + " wc-day-column-first wc-day-column-last day-" + i +'">';
              ...
              for (var j = 0; j < uLength; j++) {
                ...
                renderRow +=
                  ...
                  '">';
                renderRow +=
                  '<div class="wc-full-height-column wc-day-column-inner day-' +
                  i;
        ...
        _setupEventCreationForWeekDay: function ($weekDay) {
          ...
          //       var $newEvent = $(
          //         '<div class="wc-cal-event wc-new-cal-event wc-new-cal-event-creating"></div>'
          //       );
          ...
          //       var clickYRounded =
          //         (clickY - (clickY % options.timeslotHeight)) /
          //         options.timeslotHeight;
          //       var topPosition = clickYRounded * options.timeslotHeight;
          //       $newEvent.css({ top: topPosition });

          //       if (!options.preventDragOnEventCreation) {
          //         $target
          //           .bind("mousemove.newevent", function (event) {
          //             $newEvent.show();
          //             $newEvent.addClass("ui-resizable-resizing");
          //             var height = Math.round(
          //               event.pageY - columnOffset - topPosition
          //             );
          ...
          //               $newEvent.css(
          //                 "height",
          //                 useHeight < options.timeslotHeight
          //                   ? options.timeslotHeight
          //                   : useHeight
          //               );
          //             } else {
          //               $newEvent.css(
          //                 "height",
          //                 height + (options.timeslotHeight - remainder)
          //               );
          //             }
          //           })
          //           .mouseup(function () {
          ...
          //     if ($newEvent.length) {
          //       var createdFromSingleClick = !$newEvent.hasClass(
          ...
          //           .css({
          //             height:
          //               options.timeslotHeight * options.defaultEventLength,
          //           })
          //           .show();
          //       }
          //       var top = parseInt($newEvent.css("top"));
          //       var eventDuration = self._getEventDurationFromPositionedEventElement(
          //         $weekDay,
          //         $newEvent,
          //         top
          //       );
          ...
          //       };
          //       var showAsSeparatedUser =
          //         options.showAsSeparateUsers &&
          //         options.users &&
          //         options.users.length;

          //       if (showAsSeparatedUser) {
          //         newCalEvent = self._setEventUserId(
          //           newCalEvent,
          //           $weekDay.data("wcUserId")
          //         );
          //       } else if (
          //         !options.showAsSeparateUsers &&
          //         options.users &&
          ...
          //         newCalEvent = self._setEventUserId(
          //           newCalEvent,
          //           self._getUserIdFromIndex(0)
          //         );
          //       }

          //       var freeBusyManager = self.getFreeBusyManagerForEvent(
          //         newCalEvent
          //       );

          //       var $renderedCalEvent = self._renderEvent(
          //         newCalEvent,
          //         $weekDay
          //       );

          //       if (!options.allowCalEventOverlap) {
          //         self._adjustForEventCollisions(
          //           $weekDay,
          //           $renderedCalEvent,
          //           newCalEvent,
          //           newCalEvent
        ...
        _loadCalEvents: function (dateWithinWeek) {
          ...
          if (typeof options.data == "string") {
            ...
            jsonOptions[options.startParam || "start"] = Math.round(
              weekStartDate.getTime() / 1000
            );
            jsonOptions[options.endParam || "end"] = Math.round(
              weekEndDate.getTime() / 1000
            );
        ...
        _drawCurrentHourLine: function () {
          ...
          // then we recreate it
          var paddingStart = businessHours.limitDisplay
            ? businessHours.start
            : 0;
          ...
          var lineWidth =
            $(".wc-scrollable-grid .wc-today", this.element).width() + 3;
        ...
        _updateDayColumnHeader: function ($weekDayColumns) {
          ...
          var showAsSeparatedUser =
            ...
            options.users &&
            options.users.length;
          var todayClass = "ui-state-active user__today wc-today";

          self.element
            .find(".wc-header td.wc-day-column-header")
            .each(function (i, val) {
              $(this).html(self._getHeaderDate(currentDay));
              if (self._isToday(currentDay)) {
                $(this).addClass(todayClass);
                var para = "Hôm nay, " + $(this).text();
                $(this).text(para);
              } else {
          ...
          if (showAsSeparatedUser) {
            self.element
              .find(".wc-header td.wc-user-header")
              .each(function (i, val) {
                if (self._isToday(currentDay)) {
                  $(this).addClass(todayClass);
                } else {
                ...
                currentDay =
                  (i + 1) % options.users.length
                    ? currentDay
                    : self._addDays(currentDay, 1);
              });
          }
          currentDay = self._cloneDate(self.element.data("startDate"));
          ...
          $weekDayColumns.each(function (i, val) {
            ...
            $(this).data(
              "endDate",
              new Date(currentDay.getTime() + MILLIS_IN_DAY)
            );
            if (self._isToday(currentDay)) {
              $(this)
                .parent()
          ...
          if (options.displayFreeBusys) {
            currentDay = self._cloneDate(self.element.data("startDate"));
            self.element
              .find(".wc-grid-row-freebusy .wc-column-freebusy")
              .each(function (i, val) {
                $(this).data("startDate", self._cloneDate(currentDay));
                $(this).data(
                  "endDate",
          ...
          if (this.options.title) {
            var date = this.options.date,
              ...
              date_format = options.datetitleFormat;

            // replace the placeholders contained in the title
            title = title.replace(
              "%start%",
              self._formatDate(start, date_format)
            );
            // console.log(title);
        ...
        _renderEvents: function (data, $weekDayColumns) {
          ...
          if (data.options) {
            ...
            $.each(data.options, function (key, value) {
              if (value !== options[key]) {
                options[key] = value;
                updateLayout =
                  updateLayout || $.ui.weekCalendar.updateLayoutOptions[key];
              }
            });
            ...
            if (updateLayout) {
              ...
              self.element.empty();
              self._renderCalendar();
              $weekDayColumns = self.element.find(
                ".wc-time-slots .wc-day-column-inner"
              );
              self._updateDayColumnHeader($weekDayColumns);
              self._resizeCalendar();
              self._scrollToHour(hour, false);
          ...
          $.each(eventsToRender, function (i, calEvent) {
            ...
            var maxHour = self.options.businessHours.limitDisplay
              ? self.options.businessHours.end
              : 18;
              
            var minHour = self.options.businessHours.limitDisplay
              ? self.options.businessHours.start
              : 8;
            ...
            while (startDate < endDate) {
              ...
              if (
                ($weekDay = self._findWeekDayForEvent(
                  calEvent,
                  $weekDayColumns
                ))
              ) {
            ...
            if (start <= initialEnd) {
              ...
              if (((isMultiday && calEvent.start.getTime() != calEvent.end.getTime()) || !isMultiday) &&
                ($weekDay = self._findWeekDayForEvent(
                  calEvent,
                  $weekDayColumns
                ))
        ...
        _renderEvent: function (calEvent, $weekDay) {
          ...
          eventClass = calEvent.id
            ? "wc-cal-event"
            : "wc-cal-event wc-new-cal-event";
          if(today>end_date){
            eventHtml = '<div onclick="show_notification()" class="' + eventClass + ' ui-corner-all event__block last__event">';
          }else{
            eventHtml = '<div onclick="show_notification()" class="' + eventClass + ' ui-corner-all event__block">';
          }
        
          eventHtml += '<div class="wc-title"></div>';
          eventHtml += '<div class="wc-time ui-corner-top time__block"></div></div>';
          
          $weekDay.each(function () {
            var $calEvent = $(eventHtml);
            $modifiedEvent = options.eventRender(calEvent, $calEvent);
            $calEvent = $modifiedEvent
              ? $modifiedEvent.appendTo($(this))
              : $calEvent.appendTo($(this));
            $calEvent.css({
            ...
            $.each(groupsList, function () {
              ...
              $.each(curGroups, function (groupIndex) {
                ...
                if (self.options.overlapEventsSeparate) {
                  var newWidth =
                    self.options.totalEventsWidthPercentInOneColumn /
                    curGroups.length;
        ...
        _groupOverlappingEventElements: function ($weekDay) {
          ...
          var sortedEvents = $events.sort(function (a, b) {
            return (
              $(a).data("calEvent").start.getTime() -
              $(b).data("calEvent").start.getTime()
            );
          });
          ...
          $.each(sortedEvents, function () {
            ...
            if (curGroups.length > 0) {
              if (
                lastEndTime.getTime() <=
                $curEvent.data("calEvent").start.getTime()
              ) {
            ...
            for (
              var groupIndex = 0;
              groupIndex < curGroups.length;
              groupIndex++
            ) {
              if (curGroups[groupIndex].length > 0) {
                //checks if the event starts after the end of the last event of the group
                if (
                  curGroups[groupIndex][curGroups[groupIndex].length - 1]
                    .data("calEvent")
                    .end.getTime() <= $curEvent.data("calEvent").start.getTime()
                ) {
                  curGroups[groupIndex].push($curEvent);
                  if (
                    lastEndTime.getTime() <
                    $curEvent.data("calEvent").end.getTime()
                  ) {
            ...
            if (
              lastEndTime.getTime() < $curEvent.data("calEvent").end.getTime()
            ) {
              lastEndTime = $curEvent.data("calEvent").end;
            }
          });
        ...
        _findWeekDayForEvent: function (calEvent, $weekDayColumns) {
          ...
          $weekDayColumns.each(function (index, curDay) {
            if (
              ...
              (!showAsSeparatedUser ||
                $.inArray($(this).data("wcUserId"), user_ids) !== -1)
            ) {
              if ($weekDay) {
        ...
        _updateEventInCalendar: function (calEvent) {
          ...
          if (calEvent.id) {
            self.element.find(".wc-cal-event").each(function () {
              if (
                $(this).data("calEvent").id === calEvent.id ||
                $(this).hasClass("wc-new-cal-event")
              ) {
                $(this).remove();
                return false;
              }
          ...
          var $weekDays = self._findWeekDayForEvent(
            calEvent,
            self.element.find(".wc-grid-row-events .wc-day-column-inner")
          );
          if ($weekDays) {
        ...
        _positionEvent: function ($weekDay, $calEvent) {
          ...
          // var pxPerMillis = ($weekDay.height()+0.5) / options.millisToDisplay;
          var pxPerMillis = ($weekDay.height()) / options.millisToDisplay;
          var firstHourDisplayed = options.businessHours.limitDisplay
            ? options.businessHours.start
            : 8;
          var startMillis =
            this._getDSTdayShift(calEvent.start).getTime() -
            this._getDSTdayShift(
              new Date(
          ...
          var eventMillis =
            this._getDSTdayShift(calEvent.end).getTime() -
            this._getDSTdayShift(calEvent.start).getTime();
          ...
          $calEvent.css({
            top: pxTop,
            height:
              pxHeight || (pxPerMillis * 3600000) / options.timeslotsPerHour,
          });
        },
         ...
         * displayed calendar.
         */
        _getEventDurationFromPositionedEventElement: function (
          $weekDay,
          $calEvent,
          top
        ) {
          var options = this.options;
          var startOffsetMillis = options.businessHours.limitDisplay
            ? options.businessHours.start * 3600000
            : 8;
          var start = new Date(
            $weekDay.data("startDate").getTime() +
              startOffsetMillis +
              Math.round(top / options.timeslotHeight) *
                options.millisPerTimeslot
          );
          var end = new Date(
            start.getTime() +
              ($calEvent.height() / options.timeslotHeight) *
                options.millisPerTimeslot
          );
          return {
            start: this._getDSTdayShift(start, -1),
            end: this._getDSTdayShift(end, -1),
          };
        },
         ...
         */
        _adjustForEventCollisions: function (
          $weekDay,
          $calEvent,
          newCalEvent,
          oldCalEvent,
          ...
          $weekDay
            ...
            .each(function () {
              ...
              if (
                newCalEvent.start.getTime() < currentCalEvent.end.getTime() &&
              ...
              if (
                newCalEvent.end.getTime() > currentCalEvent.start.getTime() &&
          ...
          if (adjustedStart && maintainEventDuration) {
            newCalEvent.end = new Date(
              adjustedStart.getTime() +
                (oldCalEvent.end.getTime() - oldCalEvent.start.getTime())
            );
            self._adjustForEventCollisions(
              $weekDay,
              $calEvent,
          ...
          $weekDay.droppable({
            ...
            drop: function (event, ui) {
              ...
              if (showAsSeparatedUser) {
                ...
                var oldUserId = $(
                  ui.draggable.parents(".wc-day-column-inner").get(0)
                ).data("wcUserId");
                ...
                newCalEvent = self._setEventUserId(
                  newCalEvent,
                  userIdList.length == 1 ? userIdList[0] : userIdList
                );
              }
              self._adjustForEventCollisions(
                $weekDay,
              ...
              var $newEvent = self._renderEvent(
                newCalEvent,
                self._findWeekDayForEvent(newCalEvent, $weekDayColumns)
              );
              $calEvent.hide();

              $calEvent.data("preventClick", true);

              var $weekDayOld = self._findWeekDayForEvent(
                $calEvent.data("calEvent"),
                self.element.find(".wc-time-slots .wc-day-column-inner")
              );
        ...
        _addResizableToCalEvent: function (calEvent, $calEvent, $weekDay) {
          ...
          $calEvent.resizable({
            ...
            stop: function (event, ui) {
              ...
              // newEnd = new Date(newEnd.getTime() + diff*60000);
              if (
                self._needDSTdayShift($calEvent.data("calEvent").start, newEnd)
              )
        ...
        _refreshEventDetails: function (calEvent, $calEvent) {
          ...
          if (
            ...
            suffix =
              '<div class="wc-cal-event-delete ui-icon ui-icon-close"></div>';
          }
          $calEvent.find(".wc-time").html(this.options.eventHeader(calEvent, this.element) + suffix);
          // console.log(this.options.eventHeader(calEvent, this.element));
          $calEvent.find(".wc-title").html(this.options.eventBody(calEvent, this.element));
          $calEvent.find(".wc-title").addClass("title__");
          $calEvent.data("calEvent", calEvent);
          // this.find(".wc-cal-event").addClass("test");
        ...
        _scrollToHour: function (hour, animate) {
          ...
          var $target = this.element.find(
            ".wc-grid-timeslot-header .wc-hour-header:eq(" + slot + ")"
          );

          // $scrollable.animate({ scrollTop: 0 }, 0, function () {
          //   var targetOffset = $target.offset().top;
          //   var scroll =
          //     targetOffset - $scrollable.offset().top - $target.outerHeight();
          //   if (animate) {
          //     $scrollable.animate(
          //       { scrollTop: scroll },
        ...
        _dateFirstDayOfWeek: function (date) {
          var self = this;
          var midnightCurrentDate = new Date(
            date.getFullYear(),
            date.getMonth(),
            date.getDate()
          );
          ...
          adjustedDate.setDate(
            adjustedDate.getDate() -
              self._getAdjustedDayIndex(midnightCurrentDate)
          );
        ...
        _rotate: function (
          a /*array*/,
          p /* integer, positive integer rotate to the right, negative to the left... */
        ) {
          for (
            var l = a.length,
        ...
        _replaceChars: {
          ...
          m: function (date) {
            return (date.getMonth() < 9 ? " Tháng " : "") + (date.getMonth() + 1);
          },
        ...
        _getFreeBusyUserId: function (freeBusy) {
          ...
          if ($.isFunction(options.getFreeBusyUserId)) {
            return options.getFreeBusyUserId(
              freeBusy.getOption(),
              self.element
            );
          }
        ...
        _clearFreeBusys: function () {
          if (this.options.displayFreeBusys) {
            var self = this,
              options = this.options,
              $freeBusyPlaceholders = self.element.find(
                ".wc-grid-row-freebusy .wc-column-freebusy"
              );
        ...
        _findWeekDaysForFreeBusy: function (freeBusy, $weekDays) {
          ...
          $weekDays.each(function () {
            var manager = $(this).data("wcFreeBusyManager"),
              has_overlap =
                manager.isWithin(freeBusy.getStart()) ||
            ...
            if (
              has_overlap &&
              (!showAsSeparatedUser || $.inArray(userId, userList) != -1)
            ) {
              $returnWeekDays = $returnWeekDays
                ? $returnWeekDays.add($(this))
                : $(this);
            }
          });
        ...
        _renderFreeBusys: function (freeBusys) {
          if (this.options.displayFreeBusys) {
            ...
            $.each(freebusysToRender, function (index, freebusy) {
              var $placeholders = self._findWeekDaysForFreeBusy(
                freebusy,
                $freeBusyPlaceholders
              );
              if ($placeholders) {
        ...
        _refreshFreeBusys: function ($freeBusyPlaceholders) {
          if (this.options.displayFreeBusys && $freeBusyPlaceholders) {
            ...
            $freeBusyPlaceholders.each(function () {
              ...
              $.each(
                $placehoder.data("wcFreeBusyManager").getFreeBusys(s, e),
                function () {
        ...
        _positionFreeBusy: function ($placeholder, $freeBusy) {
          ...
          var firstHourDisplayed = options.businessHours.limitDisplay
            ...
            : 0;
          var startMillis =
            freeBusy.getStart().getTime() -
            new Date(
              freeBusy.getStart().getFullYear(),
              freeBusy.getStart().getMonth(),
              freeBusy.getStart().getDate(),
              firstHourDisplayed
            ).getTime();
          var eventMillis =
            freeBusy.getEnd().getTime() - freeBusy.getStart().getTime();
          var pxTop = pxPerMillis * startMillis;
          var pxHeight = pxPerMillis * eventMillis;
        ...
        getFreeBusyManagerForEvent: function (newCalEvent) {
          var self = this,
            ...
            freeBusyManager;
          if (options.displayFreeBusys) {
            var $freeBusyPlaceHoders = self.element.find(
                ".wc-grid-row-freebusy .wc-column-freebusy"
              ),
              freeBusy = new FreeBusy({
                start: newCalEvent.start,
                end: newCalEvent.end,
              }),
              showAsSeparatedUser =
                options.showAsSeparateUsers &&
                options.users &&
                options.users.length,
              userId = showAsSeparatedUser
                ? self._getEventUserId(newCalEvent)
                : null;
            if (!$.isArray(userId)) {
              userId = [userId];
            }
            $freeBusyPlaceHoders.each(function () {
              ...
              if (
                has_overlap &&
                (!showAsSeparatedUser ||
                  $.inArray($(this).data("wcUserId"), userId) != -1)
              ) {
        ...
        _startOnFirstDayOfWeek: function () {
          return jQuery.isFunction(this.options.startOnFirstDayOfWeek)
            ? this.options.startOnFirstDayOfWeek(this.element)
            : this.options.startOnFirstDayOfWeek;
        },
        ...
        _getHeaderDate: function (date) {
          ...
          var dayName = options.useShortDayNames
            ? options.shortDays[date.getDay()]
            : options.longDays[date.getDay()];
          return (
            dayName +
            options.headerSeparator +
            this._formatDate(date, options.dateFormat)
          );
        },
        ...
        _getDSTdayShift: function (date, shift) {
          ...
          if (offset1 == offset2) return date;
          shift = shift ? shift : 1;
          return new Date(
            date.getTime() -
              shift *
                (offset1 > offset2 ? -1 : 1) *
                (Math.max(offset1, offset2) - Math.min(offset1, offset2)) *
                60000
          );
        },
    ...
    isWithin: function (dateTime) {
      return (
        Math.floor(dateTime.getTime() / 1000) >=
          Math.floor(this.getStart().getTime() / 1000) &&
        Math.floor(dateTime.getTime() / 1000) <=
          Math.floor(this.getEnd().getTime() / 1000)
      );
    },
    isValid: function () {
  ...
  $.extend(FreeBusyManager.prototype, FreeBusyProto, {
      ...
      switch (arguments.length) {
        ...
        default:
          ...
          if (
            ...
            this.getStart().getTime() > end.getTime() ||
            this.getEnd().getTime() < start.getTime()
          ) {
            return freeBusy;
          }
          $.each(this.freeBusys, function () {
            ...
            if (
              tmpFreeBusy.isWithin(this.getStart()) &&
              tmpFreeBusy.isWithin(this.getEnd())
            ) {
              freeBusy.push(this);
            } else if (
              this.isWithin(tmpFreeBusy.getStart()) &&
              ...
              this.isWithin(tmpFreeBusy.getStart()) &&
              this.getStart().getTime() < start.getTime()
            ) {
              var _f = new FreeBusy(this.getOption());
              _f.setOption("start", tmpFreeBusy.getStart());
              freeBusy.push(_f);
    ...
    insertFreeBusy: function (freeBusy) {
      ...
      var start = freeBusy.getStart(),
        end = freeBusy.getEnd(),
        startIndex = 0,
        endIndex = this.freeBusys.length - 1,
        newFreeBusys = [];
      ...
      $.each(this.freeBusys, function (index) {
        ...
        if (curFreeBusyItem.isWithin(start) && curFreeBusyItem.isWithin(end)) {
          ...
          endIndex = index;
          if (
            start.getTime() == curFreeBusyItem.getStart().getTime() &&
            end.getTime() == curFreeBusyItem.getEnd().getTime()
          ) {
            /*
          ...
          curFreeBusyItem.isWithin(start) &&
          curFreeBusyItem.getEnd().getTime() != start.getTime()
        ) {
          /*
          ...
          curFreeBusyItem.isWithin(end) &&
          curFreeBusyItem.getStart().getTime() != end.getTime()
        ) {
          /*
                in this case, inserted freebusy starts within curFreeBusyItem:
                curFreeBusyItem:                  *----------*
